...
可以使用`show engines`来查看对应的数据支持的引擎,
![查看存储引擎.png](../../img/mysql/查看存储引擎.png)
...
一般而言，我们只需要了解两个常用的存储引擎即可，即InnoDB,MYISAM。
mysql默认支持InnoDB的存储引擎，这个可以在`my.ini`的配置文件中进行配置。
- InnoDB支持事务，行锁以及外键，且对写操作比较友好。InnoDB的存储文件只有`*.idb`,即索引和数据存储在一个文件中，因此InnoDB支持聚簇索引。
- MYISAM支持表锁，对读操作select比较友好。MYISAM的存储文件包括`*.MYI`和`*.MYD`,分别对应索引文件和数据文件，即索引和数据是分开存放的，因此MYISAM支持非聚簇索引。
...
- id: 可以代表执行语句的优先级：
  1. 当多个语句的id相同时，执行顺序按照从上到下进行
  2. 多个语句的id不同时， id值越大，执行优先级越高
- select_type: 主要来查询类型，是简单查询，还是子查询或者是联合查询。
- type: type显示的是访问类型，访问类型表示我是以何种方式去访问我们的数据，最容易想的是全表扫描，直接暴力的遍历一张表去寻找需要的数据，效率非常低下，访问的类型有很多，效率从最好到最坏依次是：  
  ...
  一般情况下，得保证查询至少达到range(范围查询 between)级别，最好能达到ref(索引)。  
  如果查询type是all,说明本次查询进行了全表扫描，index表示通过索引进行了全表扫描。 一般而言都需要对其进行优化。
- key: 重要，查看实际使用的索引
...
推荐使用代理主键，与业务不耦合，方便维护。且可以在项目中设置统一的id生成器，来减少项目的代码冗余。

## 字符集选择

1. 如果能用纯拉丁字符能表示的内容，可以选择latin1,这样可以节省时间
2. 如果确定不需要存放多种语言，就无需使用UTF-8或者其他UNICODE的字符类型。
